...
struct ScanView: View {
    ...
    @State private var leftJobBoxes: [(JobBoxRecord, TimeInterval)] = []
        ...
        ZStack {
            ...
            LinearGradient(
                gradient: Gradient(colors: [
                    Color(red: 35/255, green: 52/255, blue: 70/255),
            ...
            VStack(spacing: 0) {
                ...
                // Main scan button with modern design
                VStack(spacing: 24) {
                    ...
                    .accessibilityButton(label: "Scan Tag", hint: "Tap to scan an NFC tag")
    ...
    func processJobBoxChanges(snapshot: QuerySnapshot) {
        guard let currentUserName = sessionManager.user?.firstName else {
            return
        }
    ...
    func checkForLeftJobBoxes() {
        ...
        FirestoreManager.shared.fetchJobBoxRecords(field: "all", value: "", organizationID: orgID) { result in
            ...
            case .success(let allRecords):
                ...
                var leftJobBoxRecords: [JobBoxRecord] = []
                ...
                for (boxNumber, records) in boxGroups {
                    ...
                    if let mostRecent = records.sorted(by: { $0.timestamp > $1.timestamp }).first {
                        if mostRecent.status.lowercased() == "left job" {
                            // Check if this box belongs to the current user
                            if mostRecent.photographer.lowercased() == currentUserName.lowercased() {
                ...
                let processedRecords = leftJobBoxRecords.map { record -> (JobBoxRecord, TimeInterval) in
                    let timeDifference = currentTime.timeIntervalSince(record.timestamp)
                    let hoursInLeftJob = timeDifference / 3600.0
                    
                    print("DEBUG: Box #\(record.boxNumber) by \(record.photographer) has been in 'Left Job' for \(hoursInLeftJob) hours")
    ...
    func submitSDCardData(
        ...
        FirestoreManager.shared.saveRecord(
            ...
            case .failure(let error):
                toastMessage = "Failed to save record: \(error.localizedDescription)"
                isSuccessToast = false
                completion(false)
            }
    ...
    func submitJobBoxData(
        ...
        let wasInLeftJobStatus = lastJobBoxRecord?.status.lowercased() == "left job"
        ...
        FirestoreManager.shared.saveJobBoxRecord(
            ...
            case .success(let message):
                ...
                if wasInLeftJobStatus || wasNotificationBox {
                    // Immediately update the UI by removing this box from leftJobBoxes array
                    DispatchQueue.main.async {
            ...
            case .failure(let error):
                toastMessage = "Failed to save job box record: \(error.localizedDescription)"
                isSuccessToast = false
                completion(false)
            }
...
struct ScanView_Previews: PreviewProvider {
    static var previews: some View {
        ScanView().environmentObject(SessionManager())
    }
}
